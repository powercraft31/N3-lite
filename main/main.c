...
void app_main(void)
{
    ...
    //初始化SysTick硬件定时器（1ms周期）
    ESP_ERROR_CHECK(systick_init());
    //初始化充电桩管理组件
    InitChargeStationManager();
    //初始化0942电表管理组件
    bl0942_event_init();
    //初始化按键LED管理组件
    GPIOManager_Init();
    ...
    OrderStorage_Init();
    //注册PLC调试指令的监控
    SubscribeEvent(EVENT_PLC_SEND_RECV_DATA,PlcDeBugDataCallBack);
    //注册自动控制监控
    SubscribeEvent(EVENT_AUTO_CONTROL_MONITOR,AutoControllMonitorCallBack);
